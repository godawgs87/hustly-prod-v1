  ...
  const testEbaySyncNow = async () => {
    ...
    try {
      ...
      const {
        data: listing,
        error: listingError
      } = await supabase.from('listings').select('id, title, price, description').limit(1).single();
      ...
      const {
        data: ebayAccount,
        error: accountError
      } = await supabase.from('marketplace_accounts').select('account_username, oauth_token, oauth_expires_at, is_connected, is_active').eq('platform', 'ebay').eq('is_connected', true).maybeSingle();
      if (accountError) {
      ...
      if (!ebayAccount) {
        ...
        const {
          data: allAccounts
        } = await supabase.from('marketplace_accounts').select('platform, account_username, is_connected, is_active').eq('platform', 'ebay');
